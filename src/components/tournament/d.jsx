...
const Battle = ({ user }) => {
  ...
  const backendURL = import.meta.env.VITE_DEVELOPMENT_URL
  ...
  const handleLeaveGame = async () => {
    ...
    try {
      ...
      // ✅ Show Monetag Interstitial Ad with Custom Settings
      show_9130916({
        type: 'inApp',
        inAppSettings: {
          frequency: 1,
        ...
        try {
          show_9130916({
            ...
            inAppSettings: {
              ...
              interval: 30, // 30 sec between each
              timeout: 5, // first ad after 5 sec
              everyPage: false, // session persists across pages
            },
          })
        } catch (error) {
          console.error('Error in show_9130916:', error)
          navigate('/tournament') // fallback navigation
        }
      }, 10000) // wait 10s before triggering ads
  ...
  const handleWatchAd = async () => {
    ...
    try {
      await show_9130916().then(async () => {
        // ✅ Add 10,000 coins to user's wallet
        const userRef = ref(realtimeDB, `users/${user?.userId}/coins`)
        const snapshot = await get(userRef)
        ...
        alert('🎉 You watched an ad and received 10,000 coins!')
      })
    } catch (error) {
